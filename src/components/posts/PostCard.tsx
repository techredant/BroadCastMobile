...
export function PostCard({ post, isVisible, onDeletePost, socket, allPosts, ...otherProps }: any) {
...
{postCard.quote ? (
  ...
  <View style={{flexDirection: "column"}}>
    <Text style={{ fontWeight: "700", fontSize: 11, color:theme.text }}>
        {postCard?.reciteFirstName} 
      </Text>
      <Text style={{ fontWeight: "700", fontSize: 10, color:theme.subtext }}>@{postCard?.reciteNickName}</Text>
  </View>
...
<View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 8 }}>
  {reciteMediaList.slice(0, 4).map((uri: string, idx: number) => {
    ...
    <Text style={{ fontStyle: "italic", marginTop: 6, color:theme.text }} 
    numberOfLines={isExpanded ? undefined : 3}>
      {postCard.caption}
    </Text>
  </View>
